...
def get_socket(host, port, use_ssl, ca_cert):
    ...
    if use_ssl:
        ...
        if ca_cert is None:
            return ImpalaTSSLSocket(host, port, validate=False, ssl_version=ssl.PROTOCOL_TLSv1)
        else:
            return ImpalaTSSLSocket(host, port, validate=True, ca_certs=ca_cert, ssl_version=ssl.PROTOCOL_TLSv1)
    else:
